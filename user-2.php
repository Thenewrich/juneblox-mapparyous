...
if($loggedIn) {
        ...
        <div id="box" style="width:100%; margin-top:10px; text-align:center;">
          ...
          <?php
            ...
            ';
          ?>
          <?php //echo'<li><span style="font-weight:bold;">Profile Views: </span>';  if($loggedIn) {echo number_short($userRow->{'views'});} ?>
          </ul>
        </div>
...
<div style="margin: 10px;width: 107px;display:inline-block;float: left;"><a href="/awards/" style="color:#333;">
                <img style="border: 1px solid #000;background-color: #FDFDFD;width: 118px;height: 118px;" src="/assets/awards/3.png">
                <span style="text-align:center;display:inline-block;float:left;width: 100%;padding-left: 0;padding-right: 0;">
                  <p class="shopTitle">Admin<br></a>
                </span></a>
              </div>
  ';}
   ?>
  ...
  $friendsList = mysqli_query($conn, "SELECT * FROM `friends` WHERE  `to_id` = '$id' AND `status`='accepted' OR `from_id` = '$id' AND `status`='accepted' ORDER BY `id` DESC LIMIT 0,8");
      ...
      <h3>Friends</h3> 
          ...
          ?>
      <?php if ($friendsListCount > 8) { ?> <div style="text-align:center;padding:3px;margin-top:4px;"><a href="/friends/<?php echo $id; ?>/all" style="color: #fff;padding:2px;" class="button-style" >View All</a></div> <?php } ?>
          <style>
#friend {
    padding: 4px;
    text-align: center;
