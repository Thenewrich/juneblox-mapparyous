...
<!DOCTYPE html>
  <head>
    <title>Dashboard - Builder Land</title>
  </head>
  <body>
    <div id="body">
      <div id="column" style="float:left; width:580px;">
        <div id="box" style="width:100%;">
          <div style="margin:20px;float:left;position:relative;width: 165px;">
            <img style="width:180px" src="/avatar/render/avatars/<?php echo $userRow->{'id'}; ?>.png?c=<?php echo $userRow->{'avatar_id'}; ?>">
          </div>
...
for($c = 0; $c < 10; $c++) {
  ...
  echo '<tr style="border-bottom: 1px solid #aaaaaa;">
    <td style="padding: 8px 0px 0px 8px;background: #f8f9ff;" valign="top">'.str_replace(">","&gt;",str_replace("<","<",htmlspecialchars($statusRow['body']))).'</td><td style="background: #f8f9ff;font-size: 12.5px;font-style: italic;text-align: right;padding-top: 46px;float: right;padding-right: 5px;">'.$statusRow['date'].'</div>
    ...
    <a href="/user?id='.$statusRow['owner_id'].'" style="color:#000;text-decoration:none;">
      <img style="width:40px;" src="/avatar/render/avatars/'.$statusRow['owner_id'].'.png?c='.$posterRow['avatar_id'].'"><br>' .$posterRow['username']. '</a>
    </div></td>
    </tr>';
}
