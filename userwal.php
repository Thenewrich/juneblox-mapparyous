...
if (isset($_SESSION['id'])) {
  ...
  while($wallRow = $wall->fetch_assoc()) {
    ...
    <img style="float:left; width:80px;" src="'.$avalink.''.$wallRow['owner_id'].'.png?c='.rand()/$wallRow['avatar_id']/.'"><span style="font-size:12px;">'.str_repeat('<i class="fa fa-thumb-tack"></i>',$pin).$wallRow['time'].'</span><br>'. htmlentities ( $wallRow['post'] );
    if($loggedIn) {
      if($currentRank == 1) {
  ...
  if($count == "0"){?><em style="text-align:center;display:block;">This user has no comments</em><?php
}
